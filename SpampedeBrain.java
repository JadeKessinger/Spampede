	...
	public void updateSnake() {
		if (this.cycleNum % Preferences.REFRESH_RATE == 0) {
			...
			if (this.theData.inAImode()) {
				...
				// Reverse the snake if it's stuck
				if (this.theData.isStuck()) {
					this.theData.reverseSnake();
					this.theData.setMode_AI();

					// End game if snake is completely stuck despite reversing
					if (this.theData.isStuck()) {
						this.gameOver();
					}
				}

				// Update cell using BFS for AI mode
				nextCell = this.theData.getNextCellFromBFS();

			} else {

				// Update cell using user input when not in AI mode
