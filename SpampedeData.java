...
class SpampedeData {
	...
	public boolean isStuck() {
		BoardCell[] neighbors = this.getNeighbors(this.getSnakeHead());
	...
	public void moveSnakeForward(BoardCell cell) {
		...
		// If the cell is spam, snake grows by one cell.
		// Otherwise, we move the tail forward by one.
		if (!cell.isSpam()) {
			this.moveTailForward();
		}

		this.moveHeadForward(cell);
	}

	/**
	 * Moves the snake's head forward to the given cell.
	 * 
	 * @param cell The next cell
	 */
	private void moveHeadForward(BoardCell cell) {
		cell.becomeHead();
	...
	/**
	 * Moves the snake's tail forward by one cell
	 * 
	 */
	...
	protected BoardCell getNorthNeighbor(BoardCell cell) {
		if (cell.isWall()) {
			throw new IllegalArgumentException("Cell must not be on the boundary");
		}
		int row = cell.getRow() - 1;
		return getCell(row, cell.getColumn());
	}
	...
	protected BoardCell getEastNeighbor(BoardCell cell) {
		...
		return getCell(cell.getRow(), col);
	}
	...
	public BoardCell getNextCellInDir() {
		...
		case GOING_NORTH:
			nextCell = this.getNorthNeighbor();
			break;
		case GOING_SOUTH:
			nextCell = this.getSouthNeighbor();
			break;
		case GOING_EAST:
			nextCell = this.getEastNeighbor();
			break;
		case GOING_WEST:
			nextCell = this.getWestNeighbor();
			break;
		case AI_MODE:
			throw new IllegalStateException("Snake should not be in AI mode");
		}
	...
	public BoardCell getNextCellFromBFS() {
		...
		while (!cellsToSearch.isEmpty()) {
			BoardCell curCell = cellsToSearch.remove();

			if (curCell.isSpam()) {
				return this.getFirstCellInPath(curCell);
			}

			BoardCell[] neighbors = this.getNeighbors(curCell);
			for (BoardCell neighbor : neighbors) {
				if (!neighbor.inSearchListAlready() && neighbor.isOpen()) {
	...
	private BoardCell getFirstCellInPath(BoardCell start) {
		...
		while (!curCell.getParent().equals(this.getSnakeHead())) {
	...
	/**
	 * Reverses the linked list of snake cells
	 */
	private void reverseSnakeCells() {
		LinkedList<BoardCell> reversedSnakeCells = new LinkedList<BoardCell>();
		while (!this.snakeCells.isEmpty()) {
			reversedSnakeCells.add(this.snakeCells.removeLast());
		}
		this.snakeCells = reversedSnakeCells;
	}

	/**
	 * Calculates and sets the new direction after reversing a snake as the opposite
	 * direction from the head to the neck
	 */
	private void setReversedDirection() {
		BoardCell neck = this.getSnakeNeck();
		if (neck.equals(this.getNorthNeighbor())) {
			this.setDirectionSouth();
		} else if (neck.equals(this.getSouthNeighbor())) {
			this.setDirectionNorth();
		} else if (neck.equals(this.getEastNeighbor())) {
			this.setDirectionWest();
		} else if (neck.equals(this.getWestNeighbor())) {
			this.setDirectionEast();
		} else {
			throw new IllegalStateException("Neck not attached to head.");
		}
	}
